...
router.get('/configurations/:id/backups', async (req: Request, res: Response) => {
  try {
    ...
    const options = {
      ...
      sort: typeof sort === 'string' && ['created', 'size'].includes(sort) 
        ? sort as 'created' | 'size' 
        : undefined,
      order: typeof order === 'string' && ['asc', 'desc'].includes(order) 
        ? order as 'asc' | 'desc' 
        : undefined,
    };
...
router.get('/configurations/:configId/backups/:backupId', async (req: Request, res: Response) => {
  try {
    ...
    logger.error(`Error getting backup details: ${req.params.configId}/${req.params.backupId}`, error);
    res.status(500).json({
      success: false,
...
// Delete a backup
router.delete('/configurations/:configId/backups/:backupId', async (req: Request, res: Response) => {
  try {
    const { configId, backupId } = req.params;
...
// Restore a backup
router.post('/configurations/:configId/backups/:backupId/restore', async (req: Request, res: Response) => {
  try {
    const { configId, backupId } = req.params;
...
router.get('/configurations/:configId/backups/:backupId/files', async (req: Request, res: Response) => {
  try {
    const { configId, backupId } = req.params;
