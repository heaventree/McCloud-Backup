...
interface GoogleDriveProviderCardProps {
...
export function GoogleDriveProviderCard({ provider, onDelete }: GoogleDriveProviderCardProps) {
  ...
  useEffect(() => {
    const fetchProviderDetails = async () => {
      if (provider.type !== 'googledrive') return;
      
      try {
        setIsLoading(true);
        
        const response = await fetch(`/api/googledrive/provider/${provider.id}`);
        if (!response.ok) {
          throw new Error(`Error fetching Google Drive data: ${response.statusText}`);
        }
        ...
        setProviderDetails({
          ...
          accountInfo: {
            ...
            picture: data.accountInfo.picture
          }
        });
      } catch (error) {
      ...
      <div className="px-4 py-4 flex-grow">
        <div className="flex items-center space-x-3">
          <div className="p-2 rounded-full bg-blue-100 dark:bg-blue-600/20 text-blue-600 dark:text-blue-400">
            <SvglIcon slug="google-drive" width={20} height={20} />
          </div>
          <div>
            ...
            {isLoading ? (
              ...
              <div className="space-y-1">
                <div className="flex items-center gap-2">
                  {providerDetails.accountInfo.picture && (
                    <img 
                      src={providerDetails.accountInfo.picture} 
                      alt="Profile" 
